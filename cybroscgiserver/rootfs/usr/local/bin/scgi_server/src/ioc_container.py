...
from data_logger.data_logger_config_watcher_service import DataLoggerConfigWatcherService
...
from services.cpu_intensive_task_runner.cpu_intensive_task_runner import CPUIntensiveTaskRunner
...
from services.rw_service.subservices.plc_activity_service.plc_activity_service import \
    PlcActivityService
from services.rw_service.subservices.plc_comm_service.alc_service.alc_service import AlcService
from services.rw_service.subservices.plc_comm_service.plc_cache.plc_cache import PlcCache
from services.rw_service.subservices.plc_comm_service.plc_client_manager.plc_client_manager import \
    PlcClientManager
from services.rw_service.subservices.plc_comm_service.plc_comm_service import PlcCommService
from services.status_services.facade.plc_status_service_facade import PlcStatusServiceFacade
from services.status_services.facade.system_status_service_facade import \
    SystemStatusServiceFacade
from services.status_services.plc_status_service.plc_status_service import PlcStatusService
...
class IocContainer:
    ...
    def proxy_activity_service(self):
        if self._proxy_activity_service is None:
            self._proxy_activity_service = ProxyActivityService(self.plc_info_service,
                                                                self.repository)
    ...
    def plc_info_service(self):
        if self._plc_info_service is None:
            self._plc_info_service = PlcInfoService(get_logger(LoggerNames.PLC_INFO.name),
                                                    self.main_loop,
                                                    self.config,
                                                    )
    ...
    def alc_service(self):
        if self._alc_service is None:
            self._alc_service = AlcService(get_logger(LoggerNames.ALC.name),
                                           self.main_loop,
                                           self.config)
    ...
    def system_status_service(self):
        if self._system_status_service is None:
            self._system_status_service = SystemStatusService(self.config,
                                                              self.plc_info_service,
    ...
    def plc_status_service(self):
        if self._plc_status_service is None:
            self._plc_status_service = PlcStatusService(self.plc_info_service,
                                                        self.plc_activity_service,
                                                        self.alc_service)
    ...
    def plc_status_service_facade(self):
        if self._plc_status_service_facade is None:
            self._plc_status_service_facade = PlcStatusServiceFacade(self.plc_status_service)
    ...
    def push_service(self):
        if self._push_service is None:
            self._push_service = PushService(get_logger(LoggerNames.PUSH.name),
                                             self.main_loop,
                                             self.plc_info_service,
                                             self.plc_activity_service,
                                             self.push_activity_service,
    ...
    def plc_client_manager(self):
        if self._plc_client_manager is None:
            self._plc_client_manager = PlcClientManager(get_logger(LoggerNames.PLC_COMM.name),
    ...
    def plc_communication_service(self):
        if self._plc_communication_service is None:
            self._plc_communication_service = PlcCommService(get_logger(LoggerNames.PLC_COMM.name),
                                                             self.plc_info_service,
                                                             self.alc_service,
                                                             self.plc_activity_service,
                                                             self.plc_client_manager,
    ...
    def rw_service(self):
        if self._rw_service is None:
            self._rw_service = RWService(self.system_status_service_facade,
    ...
    def router(self):
        if self._router is None:
            self._router = Router(get_logger(LoggerNames.ABUS.name),
                                  self.communication_loop,
                                  self.push_service,
                                  self.detection_service,
                                  self.rw_service,
    ...
    def abus_transceiver(self):
        if self._abus_transceiver is None:
            self._abus_transceiver = AbusTransceiver(get_logger(LoggerNames.ABUS.name), self.router,
                                                     self.config)
    ...
    def udp_transceiver(self):
        if self._udp_transceiver is None:
            self._udp_transceiver = UdpTransceiver(get_logger(LoggerNames.UDP.name),
                                                   self.main_loop,
                                                   self.communication_loop,
    ...
    def iex_transceiver(self):
        if self._iex_transceiver is None:
            self._iex_transceiver = IexTransceiver(get_logger(LoggerNames.CAN.name),
                                                   self.abus_transceiver)
    ...
    def scgi_server(self):
        if self._scgi_server is None:
            self._scgi_server = ScgiServer(get_logger(LoggerNames.SCGI_SERVER.name),
                                           self.rw_service,
                                           self.scgi_activity_service,
                                           self.config)
    ...
    def tcp_server(self):
        if self._tcp_server is None:
            self._tcp_server = TCPServer(get_logger(LoggerNames.TCP.name),
                                         self.main_loop,
                                         self.scgi_server,
                                         self.config)
    ...
    def data_logger(self):
        if self._data_logger is None:
            self._data_logger = DataLogger(get_logger(LoggerNames.DATA_LOGGER.name),
                                           self.main_loop,
                                           self.rw_service,
                                           self.repository,
                                           self.data_logger_cache,
    ...
    def data_logger_config_watcher_service(self):
        if self._data_logger_config_watcher_service is None:
            self._data_logger_config_watcher_service = DataLoggerConfigWatcherService(get_logger(),
                                                                                      self.main_loop,
                                                                                      self.data_logger,
                                                                                      self.config)
