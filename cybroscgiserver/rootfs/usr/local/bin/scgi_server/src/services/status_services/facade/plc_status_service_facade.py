...
from services.rw_service.subservices.plc_activity_service.plc_activity import PlcActivity
...
class PlcStatusServiceFacade:
    ...
    async def _process_request(self, nad, request):
        try:
            ...
            return RResponse.create(request.name, request.tag_name, code=RResponse.Code.UNKNOWN)
...
class SinglePlcStatusServiceFacade:
    def __init__(self, single_plc_status_service):
        ...
        self._actions = {
            ...
            "alc_file_status": (
                self._alc_file_status,
                "Allocation file status."
            ),
            # "plc_status": (
            #     self._plc_status,
            #     "Controller status."
    ...
    async def _response_time(self):
        ...
        return "?" if response_time is None else f"{int(response_time.total_seconds() * 1000)}"
    ...
    async def _alc_file_status(self):
        if self._single_plc_status_service.device_status == PlcActivity.DeviceStatus.OFFLINE:
            return "-"
