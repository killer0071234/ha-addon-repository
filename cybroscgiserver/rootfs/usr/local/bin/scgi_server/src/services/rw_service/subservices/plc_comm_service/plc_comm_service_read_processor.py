...
class PlcCommServiceReadProcessor(PlcCommServiceRequestProcessor):
    def __init__(self, log, client, cpu_intensive_task_runner, on_cache_item_created=None):
    ...
    async def _process_plc_rw_requests(self, requests):
        ...
        valid = await self._process_valid(requests.one_byte, requests.two_byte, requests.four_byte)
    ...
    async def process_cache_item(self, cache_item):
        ...
        try:
            ...
            self._log.error(lambda: f"Read c{self._nad} failed with timeout", exc_info=e)
    ...
    async def _process_valid(self, one_byte, two_byte, four_byte):
        ...
        try:
            ...
            if self._on_cache_item_created is not None:
                self._on_cache_item_created((requests, command_frame_and_type_info_list))
    ...
    def _create_r_responses_with_timeout(cls, requests):
        return [
            cls._create_r_response(request, None, RResponse.Code.TIMEOUT) for request in requests
        ]
