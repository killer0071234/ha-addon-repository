...
from services.rw_service.subservices.plc_comm_service.plc_rw_request import AlcData, PlcRWRequest
from services.rw_service.subservices.plc_comm_service.plc_rw_requests import PlcRWRequests
...
class PlcCommServiceRequestProcessor(ABC):
    ...
    def _create_plc_rw_request(cls, request, alc):
        ...
        try:
            ...
            return PlcRWRequest.create(request.name, request.value, var_name, alc_data=alc_data,
                                       idx=request.idx)
        except KeyError:
            return PlcRWRequest.create(request.name, request.value, var_name, idx=request.idx)
