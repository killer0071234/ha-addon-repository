...
from services.rw_service.subservices.plc_comm_service.plc_cache.plc_cache_facade import \
    PlcCacheFacade
from services.rw_service.subservices.plc_comm_service.plc_communicator import PlcCommunicator
...
class PlcCommService:
    ...
    async def process_rw_requests(self, nad, r_requests, w_requests, task_id):
        responses = await self._process_rw_requests(nad, r_requests, w_requests, task_id)
    ...
    async def _process_rw_requests(self, nad: int, r_requests, w_requests, task_id):
        ...
        if task_id is not None:
            ...
            return await plc_communicator.process_r_requests_for_data_logger(r_requests, task_id)
        ...
        if cache_facade is None:
            ...
            responses = list(chain(cache_result.fresh.values(), cache_result.stinky.values()))
        ...
        if len(postponable_requests) > 0:
            ...
            asyncio \
                .get_running_loop() \
                .create_task(plc_communicator.process_rw_requests(postponable_requests, [])) \
    ...
    async def _get_alc(self, plc_client, crc):
        ...
        try:
            self._log.info(lambda: f"New crc for c{plc_client.plc_info.nad}: {crc}. Reload alc...")
