...
class ProxyService:
    ...
    async def _receive_async(self, received_abus_msg):
        ...
        asyncio.run_coroutine_threadsafe(self._send_async(abus_msgs), self._communication_loop)
        ...
        if session_plc_info is not None:
            ...
            if await self._repository.exist_in_relays_table(from_nad):
                ...
                if session_plc_info is not None:
                    ...
                    self._proxy_activity_service.report_from_plc(to_nad, abus_msg.from_nad)
                    return abus_msg,
                else:
                    return []
    ...
    async def _create_proxy_msgs_to_plc(self, abus_msg):
        if abus_msg.to_nad == 0:
            return [
                ProxyService.create_autodetect_acknowledge_abus_msg(
                    plc_info,
                    abus_msg
                )
