...
from services.rw_service.subservices.plc_comm_service.plc_client_manager.plc_client.file_descriptor import \
    FileDescriptor, bytes_to_file_descriptor
...
class PlcClient:
    ...
    def _create_request(self, command_frame):
        transaction_id = self._plc_info.password if self._plc_info.has_password else next(
            self._transaction_id_generator
        )
        return AbusMessage(self._addr, self._nad, self._plc_info.nad, transaction_id, command_frame)
    ...
    async def fetch_alc_file(self):
        ...
        file_descriptors = await self.read_file_descriptors(file_descriptors_addr, file_count)
        alc_file_descriptor = next(fd for fd in file_descriptors if fd.name == "alc.zip")
    ...
    async def read_plc_head(self):
        try:
            ...
            self._plc_activity_service.report_plc_head_used(self._plc_info.nad, plc_head)
            return plc_head
        except ExchangerTimeoutError as e:
    ...
    async def read_file_descriptors(self, file_descriptors_addr, file_count):
        ...
        file_descriptors_bytes = await self._read_code_memory(file_descriptors_addr, size)
        ...
        for i in range(file_count):
            ...
            file_descriptor = bytes_to_file_descriptor(file_descriptors_bytes[from_byte:to_byte])
    ...
    async def read_status(self):
        ...
        self._plc_activity_service.report_plc_status_used(self._plc_info.nad, status.plc_status)
        return status
    ...
    async def _read_random_memory_single_request(
        ...
        type_info = (one_b_items_count, two_b_items_count, four_b_items_count, four_b_types)
    ...
    async def _read_code_memory_block(self, segment_number: int, size: int):
        command_frame = CommandFrameUtil.create_read_code_memory_block(segment_number, size)
    ...
    def _generate_read_info(self, addr: int, size: int):
        ...
        for segment_number in range(first_segment_number, last_segment_number + 1):
            ...
            size = last_size if segment_number == last_segment_number else self.SEGMENT_SIZE
