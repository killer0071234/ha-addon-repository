...
class PlcCommunicator:
    ...
    async def _process_rw_requests(self, r_requests, w_requests):
        tries_taken = 0
        max_tries = 2 if self._plc_client.plc_info.origin != PlcInfo.Origin.STATIC else 1

        while tries_taken < max_tries:
            try:
                if tries_taken > 0:
                    self._plc_client = await self._update_plc_client_ip(self._plc_client)
    ...
    async def _read_for_data_logger(self, r_requests, task_id):
        try:
            ...
            return self._create_r_responses_with_code(r_requests, RResponse.Code.PLC_HEAD_ERROR)
    ...
    async def _read_write(self, r_requests, w_requests):
        ...
        if alc is None:
            return self._create_r_responses_with_code(r_requests, RResponse.Code.DEVICE_NOT_FOUND)
    ...
    def _is_plc_head_ok(cls, plc_head):
        return cls._is_plc_program_ok(plc_head) and cls._could_plc_have_alc_file(plc_head)

    @classmethod
    def _is_plc_program_ok(cls, plc_head):
        return plc_head.empty == 0 and plc_head.magic in (cls.CYBRO_2_MAGIC, cls.CYBRO_3_MAGIC)
