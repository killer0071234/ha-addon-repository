...
class SystemStatusService:
    ...
    def plc_info_table(self):
        ...
        for plc_info in self._plc_info_service.get_plc_infos():
            ...
            row = (
                ...
                None if last_failed_exchange_time is None else last_failed_exchange_time.strftime(
                    "%d-%m-%y %H:%M:%S"
                ),
                ip,
                port,
    ...
    def data_logger_status(self):
        return "active" if self._config.data_logger_config.datalogger_enabled else "stopped"
    ...
    def _tasks_to_row_data(self, tasks):
        ...
        for task in tasks:
            ...
            if last_trigger_datetime is None or \
        ...
        return (
            ...
            last_trigger_datetime.strftime(
                "%d-%m-%y %H:%M:%S"
            ) if last_trigger_datetime is not None else "",
    ...
    def proxy_activity_table(self):
        ...
        for session_id, proxy_activity in self._proxy_activity_service.extract_proxy_activities():
