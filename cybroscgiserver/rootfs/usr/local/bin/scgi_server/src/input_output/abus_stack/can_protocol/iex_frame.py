...
class IexCommand:
    ...
    def bytes_to_iex_command(iex_command):
        ...
        return IexCommand(instruction=instruction,
                          direction=direction,
                          arg=arg)

    @staticmethod
    def create_abus_stream_outgoing():
        return IexCommand(instruction=0b1010,
                          direction=0b1,
                          arg=0b010)

    @staticmethod
    def create_abus_strend_outgoing():
        return IexCommand(instruction=0b1011,
                          direction=0b1,
                          arg=0b010)
...
class IexFrame:
    ...
    def bytes_to_iex(cls, iex_header_bytes, iex_data_bytes):
        ...
        return IexFrame(command=IexCommand.bytes_to_iex_command(command),
                        address=address,
                        data=iex_data_bytes)
    ...
    def create_abus_stream_out(address, data):
        ...
        return IexFrame(command=command,
                        address=address,
                        data=data)
