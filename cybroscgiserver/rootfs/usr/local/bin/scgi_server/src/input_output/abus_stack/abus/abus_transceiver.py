...
class AbusTransceiver:
    ...
    def abus_bytes_to_iex_frames(cls, abus_msg):
        ...
        while len(abus_bytes) > 8:
            ...
            iex_frames_list.append(IexFrame.create_abus_stream_out(address=0, data=chunk))
            abus_bytes = abus_bytes[8:]
        else:
            iex_frames_list.append(IexFrame.create_abus_strend_out(address=0, data=abus_bytes))
