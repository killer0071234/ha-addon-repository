...
from constants import DATA_LOGGER_SAMPLES_TABLE, DATA_LOGGER_ALARMS_TABLE, RELAYS_TABLE
from input_output.db.model import Alarm
from input_output.db.query_builder import QueryBuilder
...
class Repository:
        ...
        queries = self._query_builder.create_insert_queries(
            ...
            [
                (
                    ...
                    '0000-00-00' if alarm.timestamp_gone is None else alarm.timestamp_gone.isoformat(sep=' ', timespec='seconds'),
                    '0000-00-00' if alarm.timestamp_ack is None else alarm.timestamp_ack.isoformat(sep=' ', timespec='seconds')
    ...
    async def create_measurements(self, measurements):
        ...
        queries = self._query_builder.create_insert_queries(
            ...
            [
                (str(m.nad), str(m.tag), str(m.value), str(m.timestamp.isoformat(sep=' ', timespec='seconds')))
    ...
    async def get_relays(self):
        ...
        query = f'SELECT user_id, enabled, session_id, message_count_rx, message_count_tx, last_message, ' \
