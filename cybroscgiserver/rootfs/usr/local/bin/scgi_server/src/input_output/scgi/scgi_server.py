...
from input_output.scgi.operation import OperationUtil
from input_output.scgi.rw_responses_xml_serializer import RWResponsesXmlSerializer
from services.rw_service.errors import ScgiCommunicationError, RWServiceError
...
class ScgiServer:
    ...
    async def _on_data(self, data):
        try:
            ...
            r_requests = [
                RWRequest.create(operation.key, operation.value) for operation in r_operations
            ]

            w_requests = [
                RWRequest.create(operation.key, operation.value) for operation in w_operations
            ]

            responses = await self._rw_service.on_rw_requests(r_requests, w_requests)

            xml = RWResponsesXmlSerializer.to_xml(responses, self._reply_with_descriptions)
