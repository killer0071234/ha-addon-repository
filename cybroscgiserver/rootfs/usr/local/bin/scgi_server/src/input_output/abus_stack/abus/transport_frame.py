...
class TransportFrameUtil:
    ...
    CRC_LENGTH = 2
    CRC_PRIM_TABLE = (
        0x049D, 0x0C07, 0x1591, 0x1ACF, 0x1D4B, 0x202D, 0x2507, 0x2B4B,
        0x34A5, 0x38C5, 0x3D3F, 0x4445, 0x4D0F, 0x538F, 0x5FB3, 0x6BBF
    )

    @classmethod
    def create_transport_frame(cls, from_addr, to_addr, data_block_bytes, transaction_id):
        transaction_id_bytes = cls._struct_transaction_id.pack(transaction_id)
        length = len(data_block_bytes) + len(transaction_id_bytes)
        header_bytes = cls._struct_header.pack(cls.SIGNATURE, length, from_addr, to_addr)
    ...
    def transport_frame_to_bytes(cls, transport_frame):
        ...
        tail = cls._struct_tail.pack(transport_frame.transaction_id, transport_frame.crc)
    ...
    def bytes_to_transport_frame(cls, transport_frame_bytes):
        ...
        max_data_block_bytes_length = len(body_bytes) - cls.TRANSACTION_ID_LENGTH - cls.CRC_LENGTH

        (signature, length, from_addr, to_addr) = cls._struct_header.unpack(header_bytes)
