...
from input_output.abus_stack.abus.command_frame import CommandFrame, Direction, CommandFrameUtil, \
    Type
...
class AbusMessage:
    ...
    def __str__(self):
        (host, port) = self.addr
        return f"{host}:{port} {self.from_nad} -> {self.to_nad} [{self.transaction_id}] " + \
...
class AbusMessageUtil:
    ...
    def abus_msg_to_bytes(cls, abus_msg):
        ...
        transport_frame_bytes = TransportFrameUtil.transport_frame_to_bytes(transport_frame)
    ...
    def create_abus_msg_from_bytes(cls, transport_frame_bytes, address):
        transport_frame = TransportFrameUtil.bytes_to_transport_frame(transport_frame_bytes)
