...
class CommandFrame:
    ...
    @property
    def size(self):
        return CommandFrameUtil.HEAD_LENGTH + \
               CommandFrameUtil.COMMAND_LENGTH + \
               len(self.body_bytes)

    def __str__(self):
        cmd_str = "CMD" if self.msg_type == 0 else f"SOCKET({self.msg_type})"
        body_str = f"{self.body_bytes.hex()}" if (len(self.body_bytes) > 0) else "no data"
...
class CommandFrameUtil:
    ...
    def create_write_random_memory(
        ...
        return cls._create_request_with_command(
            Command.WRITE_RANDOM,
            counts_bytes + addresses_bytes + value_bytes
        )
    ...
    def command_frame_to_bytes(cls, command_frame):
        head_bytes = cls._struct_head.pack(
            command_frame.msg_direction.value,
            command_frame.msg_type
        )
