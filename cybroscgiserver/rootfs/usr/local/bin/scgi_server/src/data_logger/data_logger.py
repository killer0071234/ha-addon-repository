...
from data_logger.calculate_wait_time import calculate_wait_time
from data_logger.data_logger_alarm_handler_service import DataLoggerAlarmHandlerService
from data_logger.data_logger_config_util import _extract_tasks
from data_logger.data_logger_measurement import DataLoggerMeasurement
from data_logger.data_logger_measurement_handler_service import DataLoggerMeasurementHandlerService
...
class DataLogger:
    def __init__(
        ...
        self._measurement_handler_service = DataLoggerMeasurementHandlerService(self._log, repository, cpu_intensive_task_runner)
        self._alarm_handler_service = DataLoggerAlarmHandlerService(self._log, repository)
        self._cache = cache
    ...
    def on_data_logger_config(self, data_logger_config):
        ...
        self._data_logger_activity_service.report_tasks_loaded(measurement_tasks, alarm_tasks)
    ...
    async def _handle_measurement(self, task):
        ...
        self._log.debug(lambda: f"Measurement {task.id} scheduled - execution in {wait_time}")
        ...
        valid_measurements, invalid_results_count = await self._read(task.targets, task.id)
    ...
    async def _handle_alarm(self, task) -> None:
        ...
        if invalid_results_count > 0:
            ...
            await self._alarm_handler_service.on_new_data(
                valid_measurements, task.message, task.alarm_class, task.priority, task.range, task.type
            )
    ...
    async def _read(self, targets, task_id):
        ...
        preparing_read_timedelta = timedelta(seconds=default_timer() - preparing_read_start_s)
        ...
        for request, response in zip(requests, responses):
            if response.valid:
                valid_measurements.append(
                    DataLoggerMeasurement(request.nad, response.tag_name, response.value)
                )
            else:
