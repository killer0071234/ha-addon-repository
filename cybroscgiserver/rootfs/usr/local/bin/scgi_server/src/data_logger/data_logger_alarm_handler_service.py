...
from data_logger.errors import UnexpectedValue
...
class DataLoggerAlarmHandlerService:
    ...
    async def on_new_data(self, measurements, message, alarm_class, priority, alarm_range, task_type):
        ...
        for measurement in measurements:
            try:
                not_acknowledged_alarms = not_acknowledged[measurement.nad][measurement.variable]
            except KeyError:
                not_acknowledged_alarms = []

            not_cleared_alarm_ids = [
                alarm.id for alarm in not_acknowledged_alarms if alarm.is_timestamp_gone_default
            ]
        ...
        if len(to_create_alarms) > 0:
            self._log.debug(lambda: f"Raise: " + ", ".join((str(a) for a in to_create_alarms)))

        if len(to_clear_alarm_ids) > 0:
            self._log.debug(lambda: f"Clear: " + ", ".join((str(a_id) for a_id in to_clear_alarm_ids)))
    ...
    def _is_in_range(cls, value_str, alarm_range):
        ...
        _is_in_range_with_hysteresis = low_with_hysteresis <= value <= high_with_hysteresis
