from rx import just, create
from rx.disposable import Disposable
from rx.operators import map, flat_map, do_action, filter

from config.data_logger_config.data_logger_config_xml_parser import parse_data_logger_xml
...
class DataLoggerConfigWatcherService:
    ...
    def start(self):
        ...
        if self._config.data_logger_config.datalogger_enabled:
            ...
            self._file_watcher.start(
                ...
                flat_map(
                    lambda file: just(
                        None) if file is None else self._read_data_logger_config_observable()),
                filter(lambda config: config is not None),
                do_action(log)
            ).subscribe(
                lambda config: self._data_logger.on_data_logger_config(config)
            )
        else:
            self._log.info(
                lambda: f"Watching config.ini file"
            )
            self._file_watcher.start()
    ...
    def _read_data_logger_config_observable(self):
        def subscription(observer, _):
            ...
            self._loop.create_task(task()).add_done_callback(create_task_callback(self._log))
