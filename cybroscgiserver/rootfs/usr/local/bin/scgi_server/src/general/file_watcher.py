...
from rx.operators import distinct_until_changed, debounce, observe_on
...
class FileWatcher:
    ...
    class EventHandler(FileSystemEventHandler):
        ...
        def on_any_event(self, event):
            ...
            FileWatcher.update_subject_with_file(self._timestamp, Path(event.src_path.replace("\\", "/")), self._log)
    ...
    def start(self):
        ...
        if self._config.data_logger_config.datalogger_enabled:
            return self._timestamp.pipe(
                distinct_until_changed(comparer=lambda first, second: first[1] == second[1]),
    ...
    def update_subject_with_file(cls, subject, file, log):
        ...
        if FileWatcher.configTimestamp[FileWatcher.CONFIG_PATH] != config_file_mtime \
        ...
        if old_timestamp != new_timestamp:  # !!!
            result = (
                None if new_timestamp is None else file,
                new_timestamp
            )
